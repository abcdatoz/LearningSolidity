...
select departamento, sum(total_percepciones) percp, sum(total_deducciones) dedd, sum(total) as total, sum(isr_xml) as isr
from nominas
where departamento like'%teccion%'
or departamento like'%segur%'
